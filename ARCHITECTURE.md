  ...
  <slug>:
    ...
    tasks:
      - id: <task-id>             # 一意なタスク ID
        ...
        outputs:                  # エージェントが作成・変更する成果物パターン（空なら制限なし）
          - "src/**"
...
InProgress タスクの outputs / deny.files を確認
        ↓
outputs に合致しない → {"decision":"ask","reason":"..."} を返す
...
| フック | タイミング | 処理内容 |
|--------|-----------|---------|
| `pre-edit` | Edit/Write 実行前 | outputs / deny.files チェック → block / ask |
...
main()
  └─ cmd_start(cwd, workflow_name)
      ...
      │    └─ validate(&config)   # requires の整合性チェック
      │
...
cmd_hook(cwd, event_type)
  ...
  │    ├─ 絶対パス → cwd からの相対パスに変換
  │    └─ InProgress なタスクに対して:
  │         ├─ outputs 非空 かつ非マッチ → {"decision":"ask","reason":"..."}
...
【JSON Schema の生成と整合性】
  ...
  - requires の参照整合性は JSON Schema で表現できない（クロスリファレンス）
...
| gate チェック失敗（requires 未充足） | `CompleteOutput { allowed: false, reason }` を返して処理を継続（プロセスは終了しない） |
