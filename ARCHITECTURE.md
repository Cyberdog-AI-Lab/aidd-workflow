...
│   ├── config/
│   │   ├── types.rs                     Config / Workflow / Step / Action 型定義
│   │   └── loader.rs                    YAML ロード + バリデーション
│   ├── engine/
│   │   ├── state.rs                     WorkflowState の読み書き（state.json）
│   │   ├── dag.rs                       requires 依存グラフ評価
│   │   ├── gate.rs                      gate 条件チェック
│   │   └── executor.rs                  next_actions の構築
│   ├── adapters/
│   │   └── claude_code/
│   │       └── hook_handler.rs          Claude Code フック処理（3種）
│   └── protocol/
│       ├── input.rs                     report コマンドの stdin 型
│       └── output.rs                    JSON 出力型定義
├── .workflow/
...
```json
{
  ...
  "steps": {
    ...
    "quality-check/run-test": { "status": "completed", ... },
...
並列親ステップ    → "parent-id"（サブステップから自動導出）
```
...
実行可能判定: requires の全ステップが Completed → 実行可能
```

`dag.executable_items()` はこのグラフを評価して **同時に実行可能なアイテム** を返す。並列ブロックの全サブステップは一度に実行可能として返される。
...
`hook_handler.rs` が Claude Code の 3 種類のフックイベントを処理する。
...
フックはエラーで終了しない（exit 0 固定）。ワークフロー外の操作を干渉しないように設計。
...
- [ ] `parallel` ブロックの並列実行（Agent tool の `background: true` 活用）
- [ ] `workflow-runner next` での複数ステップ同時返却の最適化
