...
approve [--callback-port|--callback-url]   稼働中の run デーモンに POST /approve
                                       （awaiting_approval を承認し、次タスクを dispatch させる）
resume [--callback-port|--callback-url]    稼働中の run デーモンに POST /resume
                                       （pause で中断したタスクを再 dispatch させる）
reject <task-id> [--reason]
       [--callback-port|--callback-url]   稼働中の run デーモンに POST /reject/:task_id
                                       （承認待ちタスクを却下してやり直す）
...
`approve` / `resume` / `reject` はいずれも稼働中の `workflow-runner run` デーモンの
コールバックサーバー（デフォルト `http://127.0.0.1:8789`）に POST するだけの薄い HTTP クライアント。
デーモンは常に `"ok"` を返す（POST が届いたことの確認のみ）ため、これらのコマンドが exit 0 で
終わっても「実際に承認/却下/再開が適用された」ことまでは保証しない — 詳細は「エラーハンドリング方針」参照。
...
`run` は 1 プロセスにつき 1 ワークフローのみを扱うため、`--workflow-id` は主に `status` で
複数ワークフローの記録が DB に残っている場合の絞り込みに使う。

```bash
# 1つしか active / awaiting_approval / paused がない場合は省略可（自動選択）
...
（オーケストレーター :8789）               （MCP サーバー :8788）    （ワーカー、常時待機）
        ...
        │    → POST :8789/approve で承認        │                        │
        │    → POST :8789/reject/:id で却下     │                        │
        │                                       │                        │
        │  ※ ワーカーが pause した場合           │                        │
        │◀── POST :8789/pause/{task_id} ────────────────────────────────│
        │    → status = paused                  │                        │
        │    → POST :8789/resume で再 dispatch  │                        │
```
...
```
POST :8789/complete/{task}（approval: true）
        ↓
...
awaiting_approval: 外部から HTTP コールバックで承認・却下を受け付ける
    ...
    承認 → POST :8789/approve     → status = 'active' → 次タスクを dispatch
    却下 → POST :8789/reject/:id  → status = 'active' → タスクを InProgress に戻して再 dispatch
```

`/approve` と `/reject/:id` はいずれも `"ok"` を即時に返す fire-and-forget な HTTP エンドポイント
（内部で `RunEvent` を mpsc チャンネルに送るだけ）。呼び出し側は「デーモンに届いた」ことしか
確認できず、「実際に awaiting_approval だったか」は保証されない。`awaiting_approval` 以外の状態で
呼ばれた場合はデーモン側で stderr にログを出して無視する。
...
`approve` と `resume` は別々の HTTP エンドポイント（`/approve` と `/resume`）で、対象外の状態
（例: `awaiting_approval` でないときに `/approve`）で呼ばれてもエラーにはならず、
デーモン側で無視されるだけ（stderr にログを出す）。
...
以下は `workflow-runner run` デーモン（`cmd/run.rs`）内部の処理フロー。`approve` / `resume` /
`reject` の各 CLI コマンドは、この状態機械そのものではなく、稼働中のデーモンへの HTTP POST を
送るだけの薄いクライアントである点に注意（デーモンを経由しないローカル状態操作は行わない）。

### `run <workflow>` 起動時

```
run_workflow(cwd, workflow_name, callback_port, callback_url, webhook_url)
...
### `POST /complete/:task_id` 受信時（`complete_task()`）
...
complete_task(cwd, workflow_id, task_id, summary, wf, config)
  ...
  │    └─ 完了なら clear_state_by_id() → プロセス終了
  │
  └─ Ok(Some(next_tasks)) → dispatch_tasks() で次タスクを webhook へ POST
```

### `POST /approve` 受信時（`RunEvent::Approve`）
...
### `POST /resume` 受信時（`RunEvent::Resume`）
...
### `POST /reject/:task_id` 受信時（`RunEvent::Reject`）
...
### `POST /pause/:task_id` 受信時（`RunEvent::Pause`）
...
| コンテキスト | 方針 |
|------------|------|
| `run` / `status` / `validate` / `list` / `setup` / `dump-schema` | `anyhow::Result` でエラーを伝播。`main()` が `ErrorOutput` JSON を stderr に出力し exit code 1 で終了 |
| `approve` / `resume` / `reject`（HTTP クライアント） | POST 自体が失敗した場合（デーモン未起動・接続不可など）のみ exit code 1 で終了。POST が届いた後の適用可否（例: awaiting_approval でなかった）はデーモン側が黙って無視するため、CLI 側からは判別できず exit 0 のまま返る |
| フックハンドラ（`cmd_hook`） | エラーでプロセスをクラッシュさせてはならない。JSON パース失敗・設定不在は `None` を返す（ワークフロー外の操作を干渉しない） |
| gate チェック失敗（requires / agents 未充足） | `run` デーモン内では stderr にログを出して dispatch をスキップし、処理を継続（プロセスは終了しない） |
