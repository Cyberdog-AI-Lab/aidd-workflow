...
- [x] **Phase 2**: `imports:` / `outputs` / `deny` / `guards` + `Action::Run` 廃止
...
| 変数定義 | `config.yml` 内の `vars:` | 別ファイルに切り出して `imports:` |
| ステップ制約 | なし | `outputs` / `deny` / `guards` |
| シェルコマンド宣言 | `actions: [{type: run}]` | `actions: [{type: agent, prompt: "..."}]` |
...
**目標**: `imports`、`outputs`、`deny`、`guards` を追加し、`Action::Run` と `checklist_key` を廃止する。
...
| `src/engine/gate.rs` | `guards` チェックを追加 |
| `src/engine/executor.rs` | `checklist_key` 参照を削除 |
| `src/protocol/output.rs` | `ResolvedAction::Manual` から `checklist_key` を削除 |
| `Cargo.toml` | `glob = "0.3"` / `regex = "1"` 追加 |
| `.workflow/config.yml` | サンプルを新スキーマに更新（`type: run` → `actions`、`checklist_key` 削除、`imports:` 例を追加） |
  ...
  bug-fix:
    ...
    steps:
      - id: design                        # checklist_key は廃止。id が一意識別子
        name: 設計確認
        outputs:                          # InProgress 中のみ有効（空なら制限なし）
          - "docs/**"
          - "/.*\.md$/"                   # / で囲まれた場合は正規表現
...
- `outputs` / `deny.files`: ファイルパスに対して評価（プロジェクトルートからの相対パス）
- `deny.commands`: 実行コマンド文字列に対して評価（部分一致）

### Action::Run の廃止移行

- `config/loader.rs` の `load_config` で `actions: [{type: run}]` を検出した場合はロードエラー
- エラーメッセージ: `"step '<id>': Action::Run は廃止されました。actions を使用してください"`
...
| フック | 判定内容 | 返却値 |
|--------|---------|--------|
| `pre-edit` | InProgress ステップの `outputs` / `deny.files` をチェック | `{"decision":"block","reason":"..."}` または `{"decision":"ask","reason":"..."}` |
